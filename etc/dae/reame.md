...
subscription {
    ...
    'file://relative/path/to/mysub.sub' # Put subscription content in /etc/dae/relative/path/to/mysub.sub curl --retry 3 --retry-delay 5 -fL -A "$UA" "$url" -o "${name}.sub.new"
    # dae/unknown (like v2rayA/1.0 WebRequestHelper) (like v2rayN/1.0 WebRequestHelper)
}
...
注意 : 配置文件和其他文件权限要求是 `0600` 或者是 `0640`
...
eg: qname(geosite:cn) -> alidns

如果 dns 是域名方式,那么就走国内alidns
...
查看日志: `sudo journalctl -u dae --output cat -f`

version="$(dae --version | head -n 1 | sed 's/dae version //')"
UA="dae/unknown (like v2rayA/1.0 WebRequestHelper) (like v2rayN/1.0 WebRequestHelper)"
curl -fL -A "dae/unknown (like v2rayA/1.0 WebRequestHelper) (like v2rayN/1.0 WebRequestHelper)" "https://mojie.co/api/v1/client/subscribe?token=d50827d5a3eda94cfefab62ef48080dc" -o "${name}.sub.new"